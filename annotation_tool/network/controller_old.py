...
def __load_lara_network__(network_path: os.PathLike) -> Tuple[Network, dict]:
    lara_path = r"C:\Users\Raphael\Desktop\pytorch_jit\traced_network.pt"

    model = torch.jit.load(lara_path, map_location="cpu")
    model.eval()
    config = {
        "sliding_window_length": 100,
    }

    print(f"Loaded network from {lara_path}")
